...
export const tool$schoolsManagementGetTeachersForSchool: ToolDefinition<
  typeof args
> = {
  name: "schools-management-get-teachers-for-school",
...
To get all Teachers for a School on the service provider. If the specified school cannot be identified within the service provider, the api will return a 404 error code and message 'School not found.'`,
  ...
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await schoolsManagementGetTeachersForSchool(
      client,
      args.request,
