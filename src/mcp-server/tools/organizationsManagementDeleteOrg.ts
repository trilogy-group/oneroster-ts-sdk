/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { organizationsManagementDeleteOrg } from "../../funcs/organizationsManagementDeleteOrg.js";
import * as operations from "../../models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.DeleteOrgRequest$inboundSchema,
};

export const tool$organizationsManagementDeleteOrg: ToolDefinition<
  typeof args
> = {
  name: "organizations-management-delete-org",
  description: `Delete an Organization

Perform a soft delete on a specific Organization on the service provider. The operation changes the status of the Organization to 'tobedeleted'.`,
  args,
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await organizationsManagementDeleteOrg(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    return formatResult(void 0, apiCall);
  },
};
