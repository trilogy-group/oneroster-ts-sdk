/*
 * Code generated by Speakeasy (https://speakeasy.com). DO NOT EDIT.
 */

import { lineItemsManagementUpdateLineItem } from "../../funcs/lineItemsManagementUpdateLineItem.js";
import * as operations from "../../models/operations/index.js";
import { formatResult, ToolDefinition } from "../tools.js";

const args = {
  request: operations.UpdateLineItemRequest$inboundSchema,
};

export const tool$lineItemsManagementUpdateLineItem: ToolDefinition<
  typeof args
> = {
  name: "line-items-management-update-line-item",
  description: `Update a Line Item

To update an existing Line Item. The sourcedId for the record to be updated is supplied by the requesting system.`,
  args,
  tool: async (client, args, ctx) => {
    const [result, apiCall] = await lineItemsManagementUpdateLineItem(
      client,
      args.request,
      { fetchOptions: { signal: ctx.signal } },
    ).$inspect();

    if (!result.ok) {
      return {
        content: [{ type: "text", text: result.error.message }],
        isError: true,
      };
    }

    return formatResult(void 0, apiCall);
  },
};
