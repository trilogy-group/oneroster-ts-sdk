...
* [getAll](#getall) - Get all courses for a school
...
async function run() {
  const result = await oneRoster.schools.courses.getAll({
    schoolSourcedId: "<id>",
  });
...
import { schoolsCoursesGetAll } from "@superbuilders/oneroster/funcs/schoolsCoursesGetAll.js";
...
async function run() {
  const res = await schoolsCoursesGetAll(oneRoster, {
    schoolSourcedId: "<id>",
  });
...
**Promise\<[operations.GetCoursesForSchoolResponse[]](../../models/.md)\>**
