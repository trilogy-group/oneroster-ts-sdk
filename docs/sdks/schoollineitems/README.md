...
* [get](#get) - Get line items for a school
...
async function run() {
  const result = await oneRoster.schoolLineItems.get({
    sourcedId: "<id>",
  });
...
import { schoolLineItemsGet } from "@superbuilders/oneroster/funcs/schoolLineItemsGet.js";
...
async function run() {
  const res = await schoolLineItemsGet(oneRoster, {
    sourcedId: "<id>",
  });
