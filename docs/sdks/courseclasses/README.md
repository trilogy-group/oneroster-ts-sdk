...
* [getAll](#getall) - Get Classes For Course
...
async function run() {
  const result = await oneRoster.courseClasses.getAll({
    courseSourcedId: "<id>",
  });
...
import { courseClassesGetAll } from "@superbuilders/oneroster/funcs/courseClassesGetAll.js";
...
async function run() {
  const res = await courseClassesGetAll(oneRoster, {
    courseSourcedId: "<id>",
  });
