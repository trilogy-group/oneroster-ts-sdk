...
* [assign](#assign) - Assign a resource to a class
...
async function run() {
  const result = await oneRoster.resources.classes.assign({
    classSourcedId: "<id>",
    requestBody: {
      resource: {
        sourcedId: "<id>",
...
import { resourcesClassesAssign } from "@superbuilders/oneroster/funcs/resourcesClassesAssign.js";
...
async function run() {
  const res = await resourcesClassesAssign(oneRoster, {
    classSourcedId: "<id>",
    requestBody: {
      resource: {
        sourcedId: "<id>",
